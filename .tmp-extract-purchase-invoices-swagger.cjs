...
const outPath = "d:/fresa/fresa-gold-frontend/.tmp-purchase-invoices-swagger-summary.json";
const TAG = "Purchase Invoices";
const REF = "$" + "ref";
...
// Always include CreatePurchaseInvoiceDto and any Purchase* DTOs
Object.keys(schemas).forEach(function(name) {
  if (name === "CreatePurchaseInvoiceDto" || /^Purchase/.test(name)) {
...
console.log("---KEY DTOs---");
["CreatePurchaseInvoiceDto"].concat(names.filter(function(n) { return /^Purchase/.test(n); })).forEach(function(n) {
  const s = relatedSchemas[n];
  if (!s || s.missing) { console.log(n + " MISSING"); return; }
  console.log(n + " required=" + JSON.stringify(s.required || []));
  if (s.properties) {
    Object.keys(s.properties).forEach(function(k) {
      const p = s.properties[k];
      const bits = [p.type || p.schemaName || "?"];
      if (p.enum) bits.push("enum=" + JSON.stringify(p.enum));
      if (p.format) bits.push(p.format);
      if (p.nullable) bits.push("nullable");
      console.log("  " + k + ": " + bits.join(" "));
    });
  }
});
console.log("---ALL relatedSchemas---");
