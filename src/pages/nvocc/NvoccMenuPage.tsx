...
export default function NvoccMenuPage() {
  ...
  const ReportsIcon = reportsNvoccTile.icon as ComponentType<{ size: number }>;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#0A2942] px-6 py-3">
        <p className="text-white text-sm">
          ...
          <span className="font-medium">NVOCC</span>
        </p>
      </div>
      ...
      <div className="p-6 grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        {/* Reports - NVOCC: centered icon layout */}
        <button
          onClick={() => navigate(reportsNvoccTile.path)}
        ...
        >
          ...
          <h3 className="text-[15px] text-[#0A2942] font-normal mb-3">
            {reportsNvoccTile.title}
          </h3>
          <p className="text-sm text-gray-500 leading-relaxed">
            {reportsNvoccTile.description}
          </p>
        </button>
      </div>
    </div>
