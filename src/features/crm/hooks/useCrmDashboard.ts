...
export const useCrmBudgets = (salespersonId?: string) => { const t = useAuthStore(s => s.accessToken); return useQuery({ queryKey: keys.budgets(salespersonId), queryFn: () => crmDashboardService.budgets(salespersonId), enabled: Boolean(t) }); };
