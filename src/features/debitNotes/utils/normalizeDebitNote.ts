...
import {
  DEBIT_NOTE_INVOICE_TYPES,
  DEBIT_NOTE_STATUSES,
  type DebitNoteInvoiceType,
  type DebitNoteStatus,
} from '../constants/debitNote.constants';
import type { DebitNote, DebitNoteLine } from '../types/debitNote.types';
...
function normalizeStatus(value: unknown): DebitNoteStatus {
  const raw = String(value ?? 'DRAFT').trim().toUpperCase();
  return (DEBIT_NOTE_STATUSES as readonly string[]).includes(raw)
    ? (raw as DebitNoteStatus)
    : 'DRAFT';
}

function normalizeType(value: unknown): DebitNoteInvoiceType | string | undefined {
  const raw = str(value)?.toUpperCase();
  if (!raw) return undefined;
  return (DEBIT_NOTE_INVOICE_TYPES as readonly string[]).includes(raw)
    ? (raw as DebitNoteInvoiceType)
    : raw;
}

export function normalizeDebitNoteLine(raw: unknown): DebitNoteLine | null {
...
export function normalizeDebitNoteLines(raw: unknown): DebitNoteLine[] {
  if (!Array.isArray(raw)) return [];
  return raw
    .map(normalizeDebitNoteLine)
...
export function normalizeDebitNote(raw: unknown): DebitNote | null {
  ...
  return {
    ...
    invoice_date: str(r.invoice_date) ?? str(r.debit_date) ?? str(r.debitDate),
    ...
    lines: normalizeDebitNoteLines(r.lines ?? r.invoice_lines),
  };
}

export function normalizeDebitNotes(raw: unknown): DebitNote[] {
  if (!Array.isArray(raw)) return [];
  return raw.map(normalizeDebitNote).filter((d): d is DebitNote => Boolean(d));
}
