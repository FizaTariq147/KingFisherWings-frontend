...
export function prepareDebitNoteLinePayload(
  dto: Record<string, unknown>,
): Record<string, unknown> {
  return prepareFields(dto, ALLOWED_LINE_FIELDS);
}

export function prepareDebitNotePayload(
  dto: Record<string, unknown>,
): Record<string, unknown> {
  ...
  if (Array.isArray(dto.lines)) {
    ...
    for (const line of dto.lines) {
      ...
      const prepared = prepareDebitNoteLinePayload(line as Record<string, unknown>);
