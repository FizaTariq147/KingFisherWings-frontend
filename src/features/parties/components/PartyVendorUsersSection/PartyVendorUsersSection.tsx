...
import {
  useCreatePartyVendorUser,
  usePartyVendorUsers,
  useResendPartyVendorInvite,
  useResetPartyVendorPassword,
  useUpdatePartyVendorUserStatus,
...
interface PartyVendorUsersSectionProps {
  partyId: string;
}

export function PartyVendorUsersSection({ partyId }: PartyVendorUsersSectionProps) {
  const { data: users = [], isLoading, isError, error, refetch } = usePartyVendorUsers(partyId);
  const createUser = useCreatePartyVendorUser(partyId);
  const updateStatus = useUpdatePartyVendorUserStatus(partyId);
  const resetPassword = useResetPartyVendorPassword(partyId);
  const resendInvite = useResendPartyVendorInvite(partyId);
  ...
  return (
    <Card>
      ...
      <div className="p-4 pt-0 space-y-3">
        ...
        {isLoading ? (
          <p className="text-sm text-[var(--color-neutral-400)]">Loading vendor users…</p>
        ) : isError ? (
          <div className="space-y-2">
            <p className="text-sm text-[var(--color-danger-600)]">
              {getErrorMessage(error) || 'Failed to load vendor users.'}
            </p>
          ...
          <p className="text-sm text-[var(--color-neutral-400)]">
            No vendor users yet. Create a login for the Vendor Payment Portal.
          </p>
        ) : (
          users.map((user) => {
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (result.temporaryPassword) {
                          ...
                          window.alert(
                            result.message || 'Password reset. Check email if SMTP is configured.',
                          );
                        }
                      } catch (err) {
      ...
      <Modal open={open} onClose={close} title="Create vendor login">
        <form
          className="space-y-3"
          noValidate
          onSubmit={form.handleSubmit(async (values) => {
            ...
            try {
              ...
              setFormError(
                getErrorMessage(err) ||
                  'Could not create vendor user. Tenant Admin / authorized staff required.',
              );
            }
          })}
        >
