...
import { PARTY_WIZARD_STEPS } from '../../constants/partyWizard.constants';

type PartyWizardStepperProps = {
  currentStep: number;
  className?: string;
};

export function PartyWizardStepper({ currentStep, className = '' }: PartyWizardStepperProps) {
  return (
    <nav aria-label="Create party steps" className={`w-full overflow-x-auto pb-1 ${className}`.trim()}>
      <ol className="relative mx-auto flex min-w-[520px] max-w-3xl items-start justify-between px-2">
        ...
        />
        {PARTY_WIZARD_STEPS.map((step, index) => {
