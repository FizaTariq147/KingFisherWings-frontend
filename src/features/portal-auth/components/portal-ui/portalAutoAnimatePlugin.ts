...
/**
 * Expressive AutoAnimate plugin — slide + scale + blur on add/remove,
 * positional FLIP on remain (filter/sort/pagination).
 */
export const portalAutoAnimatePlugin: AutoAnimationPlugin = (
  el,
  action,
  newCoords,
  oldCoords,
  ...
  // remain — FLIP between old and new positions
  if (coords && prev) {
