  ...
  return (
    <section className="rounded-[20px] bg-white p-5 shadow-[0_10px_30px_rgba(10,41,66,0.05)]">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className={dashType.panel.title}>Upcoming due</h2>
        <Link to="/vendor/schedule" className={cn(dashType.panel.linkAction, 'text-[var(--color-secondary)] hover:underline')}>
          View schedule
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <p className={cn('py-6', dashType.panel.body, 'text-[var(--color-danger-600)]')}>Could not load schedule.</p>
      ) : items.length === 0 ? (
        <p className={cn('py-6', dashType.panel.body)}>Nothing scheduled.</p>
      ) : (
            ...
            <li key={item.id}>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className={dashType.panel.rowTitle}>{item.number}</p>
...
export function VendorOpenInvoicePanel({
  ...
  return (
    <section className="flex flex-col overflow-hidden rounded-[20px] bg-[#0A2942] p-5 text-white shadow-[0_14px_34px_rgba(10,41,66,0.22)]">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className={cn(dashType.panel.title, 'text-white')}>Open invoice</h2>
        <span className={dashType.panel.meta}>{overdueCount} overdue</span>
      </div>
      ...
      {loading ? (
        ...
        <Link to={`/vendor/invoices/${item.id}`} className="block rounded-xl px-1 py-1 hover:bg-white/5">
          <p className={cn(dashType.panel.body, 'font-medium tracking-wide text-white')}>{item.number}</p>
          <div className="mt-2 flex items-start justify-between gap-3">
            <p className={dashType.panel.meta}>
