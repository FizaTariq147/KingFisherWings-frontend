...
export function preparePurchaseInvoiceLinePayload(
  dto: Record<string, unknown>,
): Record<string, unknown> {
  return prepareFields(dto, ALLOWED_LINE_FIELDS);
}

export function preparePurchaseInvoicePayload(
  dto: Record<string, unknown>,
): Record<string, unknown> {
  ...
  if (Array.isArray(dto.lines)) {
    ...
    for (const line of dto.lines) {
      ...
      const prepared = preparePurchaseInvoiceLinePayload(line as Record<string, unknown>);
