...
/** Opens leftover sample-page JSON layout PDF on select. */
export function LeftoverFormatAutoPdf({ code, autoOpen = true }: Props) {
  ...
  useEffect(() => {
    ...
    const fileName = formatPdfFilename(
      `Leftover-${layout.formatNumber}-${layout.code}`,
      'leftover-format',
    );
    let cancelled = false;
    setError(null);
