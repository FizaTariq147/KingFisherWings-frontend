...
const KIND_TONES: Partial<
  Record<OtherReportsFormatKind, { bg: string; border: string; accent: string; label: string }>
> = {
...
function matchesSearch(
  ...
  if (!q) return true;
  const hay = `${row.name} ${row.code} ${row.kind} other reports`.toLowerCase();
  return q.split(/\s+/).every((token) => hay.includes(token));
}

export function OtherReportsFormatBrowseStrip({
  selectedCode,
  onSelect,
  visible,
  searchQuery = '',
  ...
  const all = listOtherReportsFormats();
    ...
    <div className="space-y-2.5 rounded-xl border border-[var(--color-neutral-200)] bg-[var(--color-surface)] p-3.5 shadow-sm">
      ...
      {items.length === 0 ? (
        <p className="px-1 py-2 text-xs text-[var(--color-neutral-500)]">
          No Other Reports formats match “{searchQuery.trim()}”.
        </p>
      ) : (
