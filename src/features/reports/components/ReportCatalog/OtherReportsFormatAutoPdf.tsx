...
import { getOtherReportsFormatSpec } from '../../constants/otherReportsFormatCatalog';
...
export function OtherReportsFormatAutoPdf({ code, autoOpen = true }: Props) {
  const spec = getOtherReportsFormatSpec(code);
  const layout = getOtherReportsFormatUiLayout(code);
  ...
  useEffect(() => {
    ...
    const fileName = formatPdfFilename(
      `OTHER-${layout.formatNumber}-${layout.code}`,
      'other-report-format',
    );
    let cancelled = false;
    setError(null);
