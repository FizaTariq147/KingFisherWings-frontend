    ...
    <div className="space-y-2.5 rounded-xl border border-[var(--color-neutral-200)] bg-[var(--color-surface)] p-3.5 shadow-sm">
      ...
      {items.length === 0 ? (
        <p className="px-1 py-2 text-xs text-[var(--color-neutral-500)]">
          No leftover formats match “{searchQuery.trim()}”.
        </p>
      ) : (
          ...
          {items.map((row) => {
            ...
            return (
              <button
                ...
                title={row.name}
                className={[
                  'relative min-w-[8.75rem] max-w-[11rem] shrink-0 overflow-hidden rounded-lg border border-[var(--color-primary-500)]/25 bg-[var(--color-primary-50)] px-2.5 py-2.5 text-left transition',
                  active
              ...
              >
                <span
                  className="absolute inset-y-0 left-0 w-1 bg-[var(--color-primary-500)]"
                  aria-hidden
                />
                <p className="pl-1.5 text-[10px] font-bold uppercase tracking-wide text-[var(--color-primary-700)]">
                  #{row.sortOrder}
                </p>
