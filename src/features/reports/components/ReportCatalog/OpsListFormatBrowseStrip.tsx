import { listOpsListFormats } from '../../constants/opsListFormatCatalog';
...
export function OpsListFormatBrowseStrip({
  selectedCode,
  onSelect,
  visible,
  searchQuery = '',
    ...
    <div className="space-y-2.5 rounded-xl border border-[var(--color-neutral-200)] bg-[var(--color-surface)] p-3.5 shadow-sm">
      ...
      {items.length === 0 ? (
        <p className="px-1 py-2 text-xs text-[var(--color-neutral-500)]">
          No Ops List formats match “{searchQuery.trim()}”.
        </p>
      ) : (
          ...
          {items.map((row) => {
            ...
            return (
              <button
                ...
                title={row.name}
                className={[
                  'relative min-w-[8.75rem] max-w-[11rem] shrink-0 overflow-hidden rounded-lg border border-[#2286C8]/30 bg-[#E8F4FC] px-2.5 py-2.5 text-left transition',
                  active
                    ? 'ring-2 ring-[var(--color-secondary)] ring-offset-1 shadow-md'
                    : 'hover:-translate-y-0.5 hover:shadow-sm',
                ].join(' ')}
              >
                <span className="absolute inset-y-0 left-0 w-1 bg-[#2286C8]" aria-hidden />
