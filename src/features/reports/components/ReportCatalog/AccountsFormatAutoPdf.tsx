...
/**
 * Opens KingFisher Accounts format PDF on select (JSON layout preview).
 */
export function AccountsFormatAutoPdf({ code, autoOpen = true }: Props) {
  ...
  useEffect(() => {
    ...
    const fileName = formatPdfFilename(`Accounts-${spec.sortOrder}-${spec.code}`, 'accounts-format');
    let cancelled = false;
    setError(null);
