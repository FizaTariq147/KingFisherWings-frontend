...
type JobWizardStepperProps = {
  /** 0-based active step index */
  currentStep: number;
  className?: string;
};

export function JobWizardStepper({ currentStep, className = '' }: JobWizardStepperProps) {
  return (
    <nav aria-label="Create job steps" className={`w-full overflow-x-auto pb-1 ${className}`.trim()}>
      <ol className="relative mx-auto flex min-w-[640px] max-w-4xl items-start justify-between px-2">
        ...
        />
        {JOB_CREATE_WIZARD_STEPS.map((step, index) => {
