...
type TenantWizardStepperProps = {
  currentStep: number;
  className?: string;
};

export function TenantWizardStepper({ currentStep, className = '' }: TenantWizardStepperProps) {
  return (
    <nav
      aria-label="Create tenant steps"
      className={`w-full overflow-x-auto pb-1 ${className}`.trim()}
    >
      <ol className="relative mx-auto flex min-w-[520px] max-w-4xl items-start justify-between px-2">
        <li
          aria-hidden="true"
          className="pointer-events-none absolute left-[8%] right-[8%] top-3.5 h-px bg-[var(--color-neutral-200)]"
        />
        {TENANT_CREATE_WIZARD_STEPS.map((step, index) => {
