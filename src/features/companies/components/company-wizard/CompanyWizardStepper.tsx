...
export function CompanyWizardStepper({ currentStep, className = '' }: CompanyWizardStepperProps) {
  return (
    <nav
      aria-label="Create company steps"
      className={`w-full overflow-x-auto pb-1 ${className}`.trim()}
    >
      <ol className="relative mx-auto flex min-w-[420px] max-w-3xl items-start justify-between px-2">
        <li
          aria-hidden="true"
          className="pointer-events-none absolute left-[12%] right-[12%] top-3.5 h-px bg-[var(--color-neutral-200)]"
        />
        {COMPANY_CREATE_WIZARD_STEPS.map((step, index) => {
          const done = index < currentStep;
          const active = index === currentStep;
          return (
            <li
              key={step.key}
              className="relative z-[1] flex w-28 flex-col items-center text-center sm:w-36"
            >
              <span
                className={[
