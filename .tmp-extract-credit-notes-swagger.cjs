...
const outPath = "d:/fresa/fresa-gold-frontend/.tmp-credit-notes-swagger-summary.json";
const TAG = "Credit Notes";
const REF = "$" + "ref";
...
for (const pathKey of Object.keys(doc.paths || {})) {
  ...
  for (let mi = 0; mi < methods.length; mi++) {
    ...
    if (!op) continue;
    (op.tags || []).forEach(function(t) { allTags.add(t); });
...
Object.keys(schemas).forEach(function(name) {
  if (/CreditNote/i.test(name) || /^CreateCreditNote/i.test(name) || /^UpdateCreditNote/i.test(name) || /^UpdateCreditStatus/i.test(name) || /credit.?note/i.test(name)) {
    relatedSchemaNames.add(name);
  }
});
...
operations.forEach(function(o) { console.log(o.method + " " + o.path + " :: " + (o.operationId || "") + " :: " + (o.summary || "")); });
console.log("---TAGS containing credit---");
Array.from(allTags).filter(function(t) { return /credit/i.test(t); }).forEach(function(t) { console.log(t); });
console.log("---ALL CREDIT-ISH SCHEMAS---");
Object.keys(schemas).filter(function(n) { return /credit/i.test(n); }).sort().forEach(function(n) {
  const s = schemas[n];
  console.log(n + " required=[" + ((s.required||[]).join(",")) + "] props=[" + (s.properties ? Object.keys(s.properties).join(",") : "") + "]");
});
console.log("---DTO REQUIRED---");
["CreateCreditNoteDto","UpdateCreditNoteDto","CreateCreditNoteLineDto","UpdateCreditNoteLineDto","CreditNote","CreditNoteResponseDto","CreditNoteLineDto"].forEach(function(n) {
  const s = relatedSchemas[n] || schemas[n];
  if (!s) { console.log(n + ": NOT FOUND"); return; }
  console.log(n + " required=" + JSON.stringify(s.required||[]));
  if (s.properties) {
    Object.keys(s.properties).forEach(function(k) {
      const p = s.properties[k];
      const typ = p.type || p.schemaName || (p.enum ? "enum" : "?");
      const en = p.enum ? " enum=" + JSON.stringify(p.enum) : "";
      console.log("  " + k + ": " + typ + en);
    });
  }
});
